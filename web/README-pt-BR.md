...
- **RBAC na UI** — `ProtectedRoute` protege a área autenticada; `RequireScreen`
  ...
  `Forbidden` **no lugar** (o layout permanece). Duas mensagens distintas: sem
  grant de `view` → "You don't have access to this screen yet."; uma tela
  desligada (`is_enabled` off) → "This screen is temporarily unavailable." (o
  admin ignora as duas). As permissões são lidas frescas de
  `GET /me/permissions`, nunca confiadas de um token velho.
- **Controle de acesso híbrido (RBAC)** — um papel fixo (`ADMIN` ignora tudo,
  ...
  **Modules** agrupam **Screens**; os profiles carregam `is_default` (anexado no
  ...
  System + sem Delete nas tabelas admin). A **sidebar é dirigida pela
  membership** — monta suas seções a partir do `menu` de `GET /me/permissions`,
  então uma tela atribuída com zero permissões ainda aparece (rollout faseado).
  As telas de admin sob `/admin` gerenciam modules, screens (com um editor de
  permissões estilo todo por tela), profiles (uma `TransferTable` de membership
  com um `MultiSelect` de permissões por tela e a escolha de Landing) e usuários
  (atribuir profiles, desativar). Modules, screens e profiles têm cada um um
  switch **Active** (desativar, fora dos pickers de "adicionar"), e a screen tem
  também um kill switch **On** (emergência, bloqueia a tela para todos menos
  admins). Uma **tela de destino padrão** por profile mais um **override** por
  usuário decidem onde cada um cai após o login.
...
- **Academias & check-ins** — academias próximas por geolocalização + busca por
  ...
  Gestores veem a **lista completa** (sem geo, paginada) com um toggle **Show
...
- **`johndoe`** — profile `gym-member` (Dashboard, Gyms, Check-in, History).
- **`manager`** — profile `gym-manager` (as telas de academia + criar/editar
  academias + Validate Check-ins + a tela admin de Users).
- **`support`** — profile `support` (as telas Profiles/Users/Screens de controle
...
estáticos da rota, folha dinâmica publicada pelas páginas de detalhe; um crumb
único de primeira hierarquia fica cinza) e a sidebar mantém o item ativo aceso nas
sub-rotas. As páginas compartilham um **`PageHeader`** compacto (título no tamanho
da logo + descrição + ações à direita). O índice `/` resolve a tela de destino do
...
| `/admin/screens`              | `access-control.screens`  | AdminScreens       | CRUD de screens + editor de permissões por tela (ops curadas + rótulos) |
| `/admin/profiles`             | `access-control.profiles` | AdminProfiles      | CRUD de profiles                                                        |
| `/admin/profiles/:profileId`  | `access-control.profiles` | ProfileDetail      | Membership (TransferTable) + Permissões por tela + escolha de Landing   |
...
5. **Admin → Users** → abra um membro → mude role/`is_verified` → Save; confirme
   ...
   salve.
6. **Admin → Profiles** → abra um profile → restrinja a lista **Available** com o
   filtro de **module** em chips, arraste (ou use `>>`/`<<`) telas entre
   Available e Granted (cada linha mostra seu **Module**), abra o `MultiSelect`
   de **Permissions** por tela concedida para escolher suas ops curadas, escolha
   a tela de **Landing** do profile → Save. Uma tela concedida **desabilitada**
   aparece esmaecida com um badge "Disabled" e só pode ser removida em mão única
   (não dá para re-adicionar até ser reabilitada). **Modules** e **Screens**
   oferecem o mesmo CRUD para o catálogo; em **Screens**, o botão clipboard-pen
   abre o **editor de permissões** por tela (adicionar uma op curada + rótulo
   amigável, renomear uma linha, apagar uma não-system), e o dialog de edição da
   screen carrega os switches **Active** e **On**.
7. Saia e entre como **`johndoe`**, **`manager`**, **`support`** em seguida — cada
   um vê uma **sidebar diferente** montada a partir da membership do seu profile;
   o admin vê todas as seções. Visitar uma tela que você não tem renderiza
   **Forbidden** no lugar.
