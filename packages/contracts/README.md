...
| `errors.ts`      | `ERROR_CODES`, `errorCodeSchema`, `errorMetaSchema` (`{ retryAfter?, count?, action? }`), `errorResponseSchema` (`{ code, message, meta?, issues? }`) (+ `ErrorCode`/`ErrorMeta`/`ErrorResponse` types) — the shared error envelope + stable error codes                                                                                                                                                                                                |
...
| `permissions.ts` | `permissionFamilySchema` + `PERMISSION_FAMILIES`, `actionKeySchema` (free `family_name` key), `actionLabelSchema`, `actionFamily()`/`composeActionKey()`, `permissionSchema`, `createPermissionBodySchema`, `updatePermissionBodySchema`, `screenPermissionSchema` (`{ screen_key, actions: string[] }`), `menuScreenSchema` (carries `is_enabled`), `mePermissionsSchema` (+ types) — the curated permission catalog + the `GET /me/permissions` shape |
