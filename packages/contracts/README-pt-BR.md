...
| `permissions.ts` | `permissionFamilySchema` + `PERMISSION_FAMILIES`, `actionKeySchema` (chave livre `family_name`), `actionLabelSchema`, `actionFamily()`/`composeActionKey()`, `permissionSchema`, `createPermissionBodySchema`, `updatePermissionBodySchema`, `screenPermissionSchema` (`{ screen_key, actions: string[] }`), `menuScreenSchema` (carrega `is_enabled`), `mePermissionsSchema` (+ tipos) — o catálogo curado de permissões + o shape de `GET /me/permissions` |
